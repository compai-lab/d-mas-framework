...
class DLConfigurator(object):
    ...
    def start_training(self, global_model: dict = dict()):
        ...
        trained_model_state, trained_opt_state, epoch = self.trainer.train(model_state, opt_state, epoch)
        logging.info("[Configurator::train]: ################ Finished training ################")
        logging.info(f"[Configurator::train]: ################ Starting testing: from epoch {epoch} ################")
        self.trainer.test(trained_model_state, data.test_dataloader(), task='Test')
        logging.info("[Configurator::train]: ################ Finished testing ################")
        self.start_evaluations(trained_model_state, epoch)

    def start_evaluations(self, global_model, epoch):
        # Downstream Tasks
        self.downstream_tasks = []
        nr_tasks = len(self.dl_config['downstream_tasks'])
        for idx, dst_name in enumerate(self.dl_config['downstream_tasks']):
            logging.info(f"[Configurator::eval]: ################ Starting downstream task nr. "
                         f"{idx+1}/{nr_tasks} -- {dst_name}-- with model from epoch {epoch} ################")
