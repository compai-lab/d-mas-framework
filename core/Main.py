...
class Main(object):
    ...
    def setup_experiment(self):
        ...
        if configurator.dl_config['experiment']['task'] == 'train':
            ...
            configurator.start_evaluations(checkpoint['model_weights'], checkpoint['epoch'])
...
if __name__ == "__main__":
    arg_parser = add_args(argparse.ArgumentParser(description='MAD-DL'))
    ...
    config_file = None
    logging.info(
        '------------------------------- DEEP LEARNING FRAMEWORK *IML-COMPAI-DL (MAD edition) *  -------------------------------')
    try:
